...
# correct previously found lifetimes that were later determined to be slightly off
def ApplyDataCorrection(Lifetime, SysError, FitType):
    # account for LCE
    if FitType == 'SS':
        Corr,Error = FormPars.GetSSCorrectionSys()
    elif FitType == 'PoRn':
        Corr,Error = FormPars.GetPoRnCorrectionSys()
    elif FitType == 'PAX':
        Corr,Error = FormPars.GetPAXCorrectionSys()
    elif FitType == 'Field150':
        Corr,Error = FormPars.GetBeforeSR0FieldCorrectionSys()
    elif FitType == 'FieldSR0':
        Corr,Error = FormPars.GetSR0FieldCorrectionSys()
    elif FitType == 'FieldSR1':
        Corr,Error = FormPars.GetSR1FieldCorrectionSys()
    else:
        raise ValueError('strange type for data corrections!!')

    CorrectionFactor = Corr+Error*SysError
    #idea is 1/tau_v6.2.0 - 1/tau_v6.4.0 = const_eff
    TrueLifetime = Lifetime/(1 - CorrectionFactor*Lifetime)
...
def ApplyFuncCorrection(Lifetime,Indices, SysError, FitType, UnixTimes=[]):
    # account for LCE
    TrueLifetime = Lifetime
    if FitType == 'FieldSR1':
        if(len(UnixTimes)==0):
            raise ValueError('Need Unixtimes for corrections!!')
        CorrectedLifetime =  ApplyFuncCorrectionFieldSR1(Lifetime, SysError, UnixTimes)
        TrueLifetime[Indices] = CorrectedLifetime[Indices]
    else:
        if FitType == 'SS':
            Corr,Error = FormPars.GetSSCorrectionSys()
        elif FitType == 'PoRn':
        ...
        CorrectionFactor = Corr+Error*SysError
        #idea is 1/tau_v6.2.0 - 1/tau_v6.4.0 = const_eff
        TrueLifetime[Indices] = Lifetime[Indices]/(1 + CorrectionFactor*Lifetime[Indices])

    return TrueLifetime

def ApplyFuncCorrectionFieldSR1(Lifetime, SysError, UnixTimes):
    # account for LCE
    Corr,Error = FormPars.GetSR1FieldCorrectionKrSys(UnixTimes)
...
####################################
# Get Kr83m elifes  ------ !!OUTDATED!!
####################################
def LoadLifetimesKr83(PathToFile='/home/kobayashi1/work/xenon/analysis/ElectronLifetime/ElectronLifetime/FitData/Lifetimes/ElectronLifetimeDataWithKr83.txt'):
    fin = open(PathToFile)
    lines = fin.readlines()
    fin.close()
...
def LoadRawLifetimes(FitType, PathToFile):
    fin = open(PathToFile)
    lines = fin.readlines()
    fin.close()
    ...
    return (np.asarray(Unixtimes), np.asarray(UnixtimeErrors), np.asarray(ELifeValues), np.asarray(ELifeValueErrors))
...
def LoadFitData(FitType, PathToFile):
    try:
        ...
        print('\nNo lifetimes found for ' + PathToFile + '!')
    return -1

def LoadRawFitData(FitType, PathToFile):
    try:
        return LoadRawLifetimes(FitType, PathToFile)
    except:
        print('\nNo lifetimes found for ' + PathToFile + '!')
    return -1


def LoadParams(PredictionFile):
    ...
    for i, line in enumerate(lines):
        contents    = line[:-1]
        ipars    = float(contents)
        Pars.append(ipars)

    return Pars


def LoadPredictions(PredictionFile, **kwargs):
    LastPointUnixtime  = kwargs.get('LastPointUnixtime', time.time())
    DaysAfterLastPoint = kwargs.get('DaysAfterLastPoint', -1)
    fin                = open(PredictionFile)
    lines              = fin.readlines()
    fin.close()
    ...
    for i, line in enumerate(lines):
        ...
        if DaysAfterLastPoint != -1 and unixtime > LastPointUnixtime + DaysAfterLastPoint*3600.*24.:
            break
...
def ChangeElectronLifetimeForFunc(ELifes, ChangeVal, ChangeValErr, SysErr):
    #Consider that the correction line must path through the point (0.00173884  0.00165152)
    XPoint=0.00173884
    YPoint=0.001602
    NewLifetimes       = []
    for ELife in ELifes:
       Intercept, Slope = ChangeVal[0], ChangeVal[1]
       InterceptErr, SlopeErr = ChangeValErr[1], ChangeValErr[1] #Currently consider only slope error since the covariance matrix is lost
       Slope = Slope + SysErr*SlopeErr
       Intercept = YPoint - Slope*XPoint 
       NewLifetime = (1./ELife- Intercept)/Slope  # as defined in GetRelationshipBetweenRnKr.py
       NewLifetimes.append(1./NewLifetime)
    return NewLifetimes




def ChangeElectronLifetime(ELifes, ELifeUps, ELifeLows, ELifeUpErrs, ELifeLowErrs, ChangeVal, ChangeValErr, **kwargs):
    ChangeType = kwargs.get('ChangeType', 'InverseSubtract')
    ...
    for ELife, ELifeUp, ELifeLow, ELifeUpErr, ELifeLowErr in zip(
        ...
        elif ChangeType == 'LinearToInverse':
            Intercept, Slope = ChangeVal[0], ChangeVal[1]
            InterceptErr, SlopeErr = ChangeValErr[1], ChangeValErr[1]
            NewLifetime       = 1./ELife*Slope + Intercept  # as defined in GetRelationshipBetweenRnKr.py
            NewLifetimeUp     = 1./ELifeUp*Slope + Intercept
            NewLifetimeLow    = 1./ELifeLow*Slope + Intercept
            NewLifetimeUpErr = ((ELifeUpErr*Slope)**2 + (ELife*SlopeErr)**2 + InterceptErr**2)**0.5 * NewLifetime
            NewLifetimeLowErr = ((ELifeLowErr*Slope)**2 + (ELife*SlopeErr)**2 + InterceptErr**2)**0.5 * NewLifetime

            NewLifetimes.append(1./NewLifetime)
            NewLifetimeUps.append(1./NewLifetimeUp)
            NewLifetimeLows.append(1./NewLifetimeLow)
            NewLifetimeUpErrs.append(1./NewLifetimeUpErr)
            NewLifetimeLowErrs.append(1./NewLifetimeLowErr)

        else:
            print('\nChange type is not recognized - returning array of -1s!\n')
            return (-1, -1, -1, -1, -1)
...
def GetCalibrationTimes(PathToHaxDatasets, **kwargs):
    SourcesToUse = kwargs.get('SourcesToUse',['kr83m', 'rn220', 'ambe', 'neutron_generator', 'led'])
    if kwargs.get('UpdateCalibrations', False):
        print('Updating calibrations from hax')
        TagsToExclude = CommonTools.GetHaxTagsToExclude()

    HaxFileList      = PathToHaxDatasets + '*'
    HaxSortedFiles   = sorted(glob.iglob(HaxFileList), key=os.path.getctime, reverse=True)
    HaxMostRecent    = HaxSortedFiles[0]
    print(HaxMostRecent)
    dsets = pd.read_csv(HaxMostRecent)
    dsets.iloc[dsets[dsets.source__type == 'neutron_generaror'].index, dsets.columns.get_loc('source__type')] = 'neutron_generator'
    dsets.start = pd.to_datetime(dsets.start)
    dsets.end = pd.to_datetime(dsets.end)
    SourceRuns = {}
    for Source in SourcesToUse:
        print('Including ' + Source)
        SourceRuns[Source] = []
        SourceSets = dsets[dsets.source__type.str.lower() == Source]
        NewRunIndicesLow = SourceSets[(SourceSets.start.diff(1) > datetime.timedelta(days=0.5)) == True].index.tolist()
        NewRunIndicesUp = SourceSets[(SourceSets.start.diff(-1) < -datetime.timedelta(days=0.5)) == True].index.tolist()
        NewRunIndicesLow.insert(0, SourceSets.index.min())
        NewRunIndicesUp.append(SourceSets.index.max())
        for IndexLow, IndexUp in zip(NewRunIndicesLow, NewRunIndicesUp):
            # check for NaT in start and end timestamps
            for i in range(IndexUp - IndexLow):
                try:
                    if type(SourceSets.loc[IndexLow].start) == pd.tslib.Timestamp:
                        break
                except:
                    pass
                IndexLow += 1
                print(Source, IndexLow)
            for i in range(IndexUp - IndexLow):
                try:
                    if type(SourceSets.loc[IndexUp].end) == pd.tslib.Timestamp:
                        break
                except:
                    pass
                IndexUp -= 1
                print(Source, IndexUp)

            if IndexUp <= IndexLow:
                continue

            StartTime = SourceSets.loc[IndexLow].start
            EndTime = SourceSets.loc[IndexUp].end
            SourceRuns[Source].append([StartTime, EndTime])

    return SourceRuns


def GetpValuesAndBayesFactor(DataUnixtimes, DataELifeValues, DataELifeValueErrors, PredictionInterpMedian, **kwargs):
    NumIterations = kwargs.get('NumIterations', 1)
    TrueLnLs = np.zeros((len(DataUnixtimes), NumIterations))
    DeltaLnLs = np.zeros((len(DataUnixtimes), NumIterations))
    for i,(Unixtime,Lifetime,LifetimeErr) in enumerate(zip(DataUnixtimes, DataELifeValues, DataELifeValueErrors)):
        for j in range(NumIterations):
            TrueLnL = 0.
            PredictionLnL = 0.

            # compute true lnl between data and prediction
            DeltaELife = Lifetime - PredictionInterpMedian(Unixtime)
            TrueLnL = -0.5 * ((DeltaELife / LifetimeErr)**2 + np.log(2*np.pi*LifetimeErr))

            # compute lnl between "fake data" and prediction
            rand = np.random.normal(0, 1)
            FakeELife = rand * LifetimeErr + PredictionInterpMedian(Unixtime)
            DeltaELifeFake = FakeELife - PredictionInterpMedian(Unixtime)
            PredictionLnL = -0.5 * ((DeltaELifeFake / LifetimeErr)**2 + np.log(2*np.pi*LifetimeErr))

            TrueLnLs[i][j] = TrueLnL
            DeltaLnLs[i][j] = TrueLnL - PredictionLnL

    pValue = float(np.where(DeltaLnLs > 0)[0].shape[0]) / float(DeltaLnLs.flatten().shape[0])
    K = np.average(np.exp(TrueLnLs))
    return (TrueLnLs, DeltaLnLs, pValue, K)
